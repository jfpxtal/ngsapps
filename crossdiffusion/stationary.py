...
def AApply(uv, p, mesh, mat):
    ...
    mmr = Integrate(IfPos(
        -max(a, b),
        exp(a)/(1+exp(a)+exp(b)),          # a <= 0 && b <= 0
        IfPos(
            a-b,
            1/(1+exp(-a)+exp(b-a)),        # a >= b && a > 0
            exp(a-b)/(1+exp(-b)+exp(a-b))  # b > a && b > 0
            )
        ), mesh, definedon=mat)

    mmb = Integrate(IfPos(
        -max(a, b),
        exp(b)/(1+exp(a)+exp(b)),          # a <= 0 && b <= 0
        IfPos(
            a-b,
            exp(b-a)/(1+exp(-a)+exp(b-a)), # a >= b && a > 0
            1/(1+exp(-b)+exp(a-b))         # b > a && b > 0
            )
        ), mesh, definedon=mat)

    mmr2 = Integrate(1 / (1+exp(-(uv[0]-p.Vr)/p.Dr)+exp((uv[1]-p.Vb)/p.Db-(uv[0]-p.Vr)/p.Dr)), mesh, definedon=mat)
    mmb2 = Integrate(1 / (1+exp((uv[0]-p.Vr)/p.Dr-(uv[1]-p.Vb)/p.Db)+exp(-(uv[1]-p.Vb)/p.Db)), mesh, definedon=mat)
    err = sqrt((mmr-mmr2)**2+(mmb-mmb2)**2)
    if err > 0:
        print(err)
